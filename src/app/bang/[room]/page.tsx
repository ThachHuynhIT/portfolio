...
const BangTable = dynamic(() => import("@/components/bang/BangTable"), {
  ssr: false,
  loading: () => <p className="animate-pulse pt-32 text-center text-amber-100/70">Đang tải bàn chơi…</p>,
});

export default function BangRoomPage({ params }: { params: { room: string } }) {
  ...
  if (!name) {
    return (
      <main className="mx-auto flex min-h-[100dvh] max-w-sm flex-col justify-center px-4">
        <Link href="/bang" className="mb-6 self-start text-sm text-amber-100/70 hover:text-amber-50">
          ← Sảnh
        </Link>
        <h1 className="mb-1 text-2xl font-black text-amber-300">
          {watch ? "Xem" : "Vào"} bàn {code}
        </h1>
        <p className="mb-5 text-sm text-amber-100/70">Nhập tên để mọi người nhận ra bạn.</p>
        <NameForm
          initial=""
  ...
  return <BangTable code={code} name={name} watch={watch} />;
}
