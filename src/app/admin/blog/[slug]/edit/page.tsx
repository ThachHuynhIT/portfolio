...
export default function EditBlogPostPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(t.admin.blog.toastUpdated);
  ...
  return (
    <div>
      <AdminHeader
        title={`${t.admin.common.edit}: ${title}`}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className={cn("p-6", radius.card, "bg-gray-900 border border-gray-800 space-y-4")}>
          ...
          {contentLang === "en" ? (
            <div className={cn("grid grid-cols-1 md:grid-cols-2", gap.loose)}>
              ...
              <FormField label={`${t.admin.blog.sharedUrlSlug} (${t.admin.common.optional})`} id="post-slug">
                <input
                  id="post-slug"
                  type="text"
                  value={slugParam}
                  disabled
                  className={cn("w-full px-4 py-2 bg-gray-950 border border-gray-800", radius.control, "text-gray-500 font-mono cursor-not-allowed")}
                />
              </FormField>
            </div>
          ) : (
        ...
        <AdminFormFooter
          ...
          saveLabel={t.admin.blog.editTitle}
          isSaving={saving}
        />
      </form>
