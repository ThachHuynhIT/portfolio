...
const MESSAGE_LIST_TITLE = 'QR Xmas Tree';
...
export default function LetterList({ encryptedData }: IHeartListProps) {
    ...
    const messageColumns: ColumnConfig<IHeart>[] = [
        ...
        },
        {
            ...
            render: (item: IHeart, index: number) => {
                return (
                    <a key={index} href={route('xmas-tree-result', { id: item.id })}>
                        <QRCode key={`xmas-tree-${index}`} value={`${window.location.origin}/xmas-tree/${item.id}`} size={100} />
                    </a>
                );
            },
        },
        {
            ...
            render: (item: IHeart, index: number) => {
                return (
                    <div key={index}>
                        <a
                            href={route('edit-xmas-tree-form', { id: item.id })}
    ...
    const fetchClients = useCallback(async () => {
        try {
            const query: string = `/admin/xmas-tree-filter?search=${encodeURIComponent(searchText ?? '')}&page=${pagination.currentPage}&per_page=${pagination.itemsPerPage}&qr=${searchQR ?? ''}`;
