...
export default function CompareLayout({ photos, onSelectPhoto }: CompareLayoutProps) {
  ...
  if (comparablePhotos.length === 0) {
    return (
      <div className="text-center py-20 bg-white/[0.02] border border-white/10 rounded-2xl p-8 light:bg-neutral-900/[0.03] light:border-neutral-900/10">
      ...
      {comparablePhotos.map((photo, index) => {
        ...
        return (
          <motion.div
            ...
            className="rounded-3xl p-6 md:p-8 bg-slate-900/40 border border-white/10 shadow-xl light:bg-white light:border-neutral-900/10 light:shadow-neutral-400/10"
          >
            {/* Header info */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div>
                ...
                <h3 className="text-xl md:text-2xl font-bold text-white light:text-neutral-900">{title}</h3>
              </div>

              <button
                type="button"
                onClick={() => onSelectPhoto(photo)}
                className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-white/10 hover:bg-white/20 text-white text-xs font-semibold border border-white/15 transition-all self-start md:self-auto light:bg-neutral-900/[0.06] light:hover:bg-neutral-900/10 light:text-neutral-900 light:border-neutral-900/10"
              >
            ...
            {(hasEditingInfo || hasCameraInfo) && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs pt-4 border-t border-white/10 light:border-neutral-900/10">
                ...
                {hasCameraInfo && (
                  <div className="rounded-xl bg-white/[0.03] border border-white/10 p-4 flex flex-col justify-between light:bg-neutral-900/[0.03] light:border-neutral-900/10">
                    <span className="font-semibold text-cyan-400 uppercase tracking-wider block mb-1 text-[10px] light:text-cyan-700">
                      Technical Specs
                    </span>
                    <div className="grid grid-cols-2 gap-2 text-slate-300 light:text-neutral-600">
