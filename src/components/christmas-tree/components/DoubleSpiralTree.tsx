...
function createSnowflakeTextureFromSVG(size = 256, fill = '#4a90e2') {
  const c = document.createElement('canvas');
  c.width = c.height = size;
  const ctx = c.getContext('2d')!;
...
export default function DoubleSpiralTree3D({
  ...
  const snowTex = useMemo(() => createSnowflakeTextureFromSVG(256, '#4a90e2'), []);
  ...
  const pOrder = useMemo(() => {
    const aIdx = pParams.map((_, i) => i).filter(i => pParams[i].which === 0).sort((i, j) => pParams[i].u - pParams[j].u);
    const bIdx = pParams.map((_, i) => i).filter(i => pParams[i].which === 1).sort((i, j) => pParams[i].u - pParams[j].u);
    const order: number[] = []; let ia = 0, ib = 0;
    while (ia < aIdx.length || ib < bIdx.length) {
      if (ia >= aIdx.length) { order.push(bIdx[ib++]); continue; }
      if (ib >= bIdx.length) { order.push(aIdx[ia++]); continue; }
      const ua = pParams[aIdx[ia]].u; const ub = pParams[bIdx[ib]].u;
      if (ua <= ub) order.push(aIdx[ia++]); else order.push(bIdx[ib++]);
    }
    return order;
  }, [pParams]);
  ...
  const pCol = useMemo(() => {
    ...
    for (let i = 0; i < N; i++) {
      const src = pParams[pOrder[i]]; const t = src.u; const taper = 1 - t;
      const c = color.clone().multiplyScalar(0.95 + 0.25 * taper + Math.random() * 0.05);
      a[i * 3 + 0] = c.r; a[i * 3 + 1] = c.g; a[i * 3 + 2] = c.b;
    }
    return a;
  ...
  const center = new THREE.Vector3(), tangent = new THREE.Vector3(), normal = new THREE.Vector3(), binormal = new THREE.Vector3(), tmp = new THREE.Vector3();
  ...
  const lightningBoltObjs = useMemo(() => {
    const arr: {
      ...
      curN: Float32Array; curB: Float32Array;
      velN: Float32Array; velB: Float32Array;
      tgtN: Float32Array; tgtB: Float32Array;
      nextT: Float32Array;
      freq: Float32Array;
      seed: Float32Array;
    }[] = [];
  ...
  useMemo(() => {
    for (let i = 0; i < N; i++) {
      ...
      tmp.copy(normal).multiplyScalar(Math.cos(src.ph) * src.r);
      tmp.addScaledVector(binormal, Math.sin(src.ph) * src.r);
      ...
      center.add(tmp);
      pPos[i * 3 + 0] = center.x; pPos[i * 3 + 1] = center.y; pPos[i * 3 + 2] = center.z;

      pVel[i * 3 + 0] = 0; pVel[i * 3 + 1] = 0; pVel[i * 3 + 2] = 0;
    }
  ...
  useFrame((state) => {
    ...
    if (starRef.current && !vanishedRef.current) starRef.current.lookAt(state.camera.position);
    ...
    if (!vanishedRef.current) {
      ...
      lightningBoltObjs.forEach((obj, k) => {
        ...
        for (let i = 0; i < totalVertices; i++) {
          ...
          const baseOffset = offset_dir * tubeRadius * (2.2 + 1.2 * Math.sin(t_curve * 5.0 + k_phase)) * L_AMP_MUL;
          const p = tmp.copy(center)
      ...
      if (explodeStartRef.current !== null && pGeoRef.current) {
        ...
        for (let i = 0; i < N; i++) {
          ...
          pVel[j + 0] *= drag; pVel[j + 1] *= drag; pVel[j + 2] *= drag;
          arr[j + 0] += pVel[j + 0] * dt;
          arr[j + 1] += pVel[j + 1] * dt;
          arr[j + 2] += pVel[j + 2] * dt;
        }
  ...
  return (
    <>
      ...
      <instancedMesh
        ref={snowMeshRef}
        args={[undefined as any, undefined as any, snowCount]}
        frustumCulled={false}
        renderOrder={1}
      >
      ...
      <group ref={groupRef}>
        ...
        <points
          ...
          ref={(p) => { if (p) pGeoRef.current = p.geometry as THREE.BufferGeometry; }}
        >
          <pointsMaterial
            ref={pMatRef}
            size={0.024}
