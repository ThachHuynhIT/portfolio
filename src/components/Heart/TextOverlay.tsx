...
export type TextOverlayProps = {
  ...
  /** Reduce rendering when heart animation is active */
...
export default function TextOverlay({
  ...
  // Auto-start text animation after 3 seconds delay
  useEffect(() => {
  ...
  useEffect(() => {
    if (!mountRef.current || !shouldShowText) return;
    ...
    const initializeRenderer = async () => {
      ...
      const renderer = new THREE.WebGLRenderer({
        antialias: false, // disable on medium
        alpha: true,
        ...
        failIfMajorPerformanceCaveat: false, // removed low tier check
      });

      // Adaptive pixel ratio and quality settings
      let baseDPR = Math.min(window.devicePixelRatio || 1, 1.5);
      ...
      const createWordSprite = (word: string, colorStr: string, px: number): WordSprite => {
        ...
        else if (sw < 1024) adjusted = Math.max(px * 0.9, 80);
        ...
        if (!c) {
          ...
          // Simplified rendering based on tier
          if (tier === "medium") {
            // Moderate 3D effect for medium devices
            // Reduced bevel layers
            const bevelLayers = [
              { x: 2, y: 2, color: "rgba(0,0,0,0.5)", blur: 0 }, // Shadow
              { x: 1, y: 1, color: "rgba(100,100,100,0.3)", blur: 0 }, // Highlight
            ];
            ...
            // Main text with simple gradient
            const gradient = ctx.createLinearGradient(0, padding, 0, padding + adjusted);
            try {
              gradient.addColorStop(0, colorStr);
              gradient.addColorStop(1, adjustBrightness(colorStr, -20));
            } catch (error) {
              gradient.addColorStop(0, colorStr);
            ...
            // Full 3D effect for high-end devices (existing code)
      ...
      const createParticlesFromWord = (wordSprite: WordSprite, particleCount: number = 20) => {
        ...
        // Reduce particle count for medium tier devices and when heart is animating
        let adjustedParticleCount = particleCount;
        if (tier === "medium") adjustedParticleCount = Math.floor(particleCount * 0.7); // 70% particles

        // if (isHeartAnimating) {
        //   adjustedParticleCount = Math.floor(adjustedParticleCount * 0.6); // Further reduce when heart is active
        // }
        ...
        for (let i = 0; i < adjustedParticleCount; i++) {
          ...
          particle.initialScale = 3 + Math.random() * 4;
      ...
      const buildSentence = (sentence: string) => {
        ...
        if (tier === "medium") adjustedMaxLineWidth *= 0.95;
      ...
      // ---- Dynamic Resolution Scaling (DRS) & frame skipping ----
      const fps = new FPS();
      let dpr = baseDPR;

      // More aggressive frame skipping when heart is animating
      let baseFrameSkip = 1;
      // if (isHeartAnimating) {
      //   baseFrameSkip = 2;
      // }

      let frameSkip = baseFrameSkip;
      let skip = 0;

      const animate = () => {
        rafId = requestAnimationFrame(animate);

        // Enhanced frame skip logic with heart animation consideration
        if (frameSkip && (skip = (skip + 1) % (frameSkip + 1))) {
        ...
        if (phase === "move") {
          ...
          // During hold: render only first frame and then reduce frequency based on tier and heart animation
          if (phaseTime === dt) needsRender = true; // first tick of hold
          else {
            const holdRenderInterval = isHeartAnimating ? (tier === "medium" ? 150 : 100) : tier === "medium" ? 80 : 50;
            if (Math.floor(phaseTime / holdRenderInterval) % 10 === 0) needsRender = true;
          }
          if (phaseTime >= holdDurationMs) {
            ...
            for (let i = 0; i < wordSprites.length; i++) {
              ...
              // Create particles with adjusted count based on performance tier
              const baseParticleCount = 18;
              const wordParticles = createParticlesFromWord(s, baseParticleCount);
        ...
        // Measure & adapt DPR with consideration for heart animation
        fps.tick();
        if ((performance.now() | 0) % 500 < 17) {
          const targetFPS = 30;
          const highFPS = 50;

          if (fps.fps < targetFPS && dpr > 0.5) {
            dpr = Math.max(0.5, dpr - 0.1);
            renderer.setPixelRatio(dpr);
            renderer.setSize(mountRef.current?.clientWidth ?? 1, mountRef.current?.clientHeight ?? 1, false);
          } else if (fps.fps > highFPS && dpr < baseDPR) {
            dpr = Math.min(baseDPR, dpr + 0.05); // More conservative increase when heart is active
            renderer.setPixelRatio(dpr);
            renderer.setSize(mountRef.current?.clientWidth ?? 1, mountRef.current?.clientHeight ?? 1, false);
          }
        }
    ...
    initializeRenderer();
  }, [shouldShowText]); // Add shouldShowText as dependency

  return <div ref={mountRef} className={styles.textGlowMedium} style={{ ...layerStyle, ...style }} />;
}
