...
- **`/meo-no`, `/co-ty-phu`, `/splendor` (+ `/[room]`)**: Mèo Nổ, Cờ Tỷ Phú (Monopoly with Vietnamese places) and Đá Quý (Splendor-style, original SVG artwork) on the same be_game backend (`/api/meono/*`, `/api/typhu/*`, `/api/splendor/*`), sharing `src/components/games/` (lobby, WebSocket hook, chat, rank-points picker). `src/lib/meono/`, `src/lib/typhu/`, `src/lib/splendor/` are client copies of be_game's data and protocol.
